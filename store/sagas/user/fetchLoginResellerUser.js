...
async function fetchLoginResellerUserApi(userInfo) {
  const response = await fetch(
    `${process.env.USER_API_ENDPOINT}/resellers/auth/login`,
    {
      method: 'POST',
      mode: 'cors',
      cache: 'no-cache',
...
export default function* asyncLoginAdminUser(action) {
  try {
    if (window !== undefined) {
      ...
      yield put({
        type: 'SUCCESS_FETCH_LOGIN_RESELLER_USER',
    ...
    localStorage.removeItem('accessToken');
    localStorage.removeItem('refreshToken');
    yield put({ type: 'FAILURE_FETCH_LOGIN_RESELLER_USER' });
  }
}
